...
const GetStakedInfo: React.FC<LbryRatioProps> = ({ actorSwap, isAuthenticated }) => {
    ...
    return (<div className="account-wrapper">
        <div>Aether Staked :{swap.stakeInfo.stakedAether} AETHER {isAuthenticated === true ? (<Unstake actorSwap={actorSwap} />) : <button className=" w-full rounded-full text-center text-black border-solid border bg-black border-black mt-8"><Auth /></button>}  </div>
