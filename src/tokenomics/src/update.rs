...
const aether_canister_id: &str = "7hcrm-4iaaa-aaaak-akuka-cai";
...
pub async fn mint_AETHER(lbry_burn: u64, actual_caller: Principal) -> Result<String, String> {
    let mut minted_aether: u64 = 0;
    ...
    if (total_burned_lbry + lbry_burn) > LBRY_THRESHOLDS[LBRY_THRESHOLDS.len() - 1] {
        return Err("Max AETHER reached,minting stopped !".to_string());
    }
    ...
    if tentative_total > (LBRY_THRESHOLDS[current_threshold as usize]) {
        ...
        while tentative_total > (LBRY_THRESHOLDS[current_threshold as usize]) {
            ...
            let mut lbry_mint_aether_with_current_threshold: u64 =
                LBRY_THRESHOLDS[current_threshold as usize];
            ic_cdk::println!(
                "lbry_mint_aether_with_current_threshold is {}",
                lbry_mint_aether_with_current_threshold
            );

            lbry_mint_aether_with_current_threshold = lbry_mint_aether_with_current_threshold
                .checked_sub(total_burned_lbry)
                .ok_or("Arithmetic underflow occurred in lbry_mint_aether_with_current_threshold")?;
           
            let mut phase_mint_aether: u64 = (AETHER_PER_THRESHOLD[current_threshold as usize])
                .checked_mul(lbry_mint_aether_with_current_threshold)
                .ok_or("Arithmetic overflow occurred in phase_mint_aether")?;

            phase_mint_aether = phase_mint_aether
                .checked_mul(10000)
                .ok_or("Arithmetic overflow occurred in phase_mint_aether.")?;

            mint_AETHER_internal(phase_mint_aether, actual_caller).await?; //mint to actual caller
            mint_AETHER_internal(
                phase_mint_aether,
            ...
            phase_mint_aether = phase_mint_aether
                .checked_mul(3)
                .ok_or("Arithmetic overflow occurred in phase_mint_aether")?;
            minted_aether = minted_aether
                .checked_add(phase_mint_aether)
                .ok_or("Arithmetic overflow occurred in minted_aether")?;
            lbry_processed = lbry_processed
                .checked_add(lbry_mint_aether_with_current_threshold)
        ...
        if lbry_burn > lbry_processed {
            let lbry_mint_aether_with_current_threshold: u64 = lbry_burn
        ...
        let mut phase_mint_aether = AETHER_PER_THRESHOLD[current_threshold as usize]
            .checked_mul(lbry_burn)
            .ok_or("Arithmetic overflow occurred in phase_mint_aether")?;
        phase_mint_aether = phase_mint_aether
            .checked_mul(10000)
            .ok_or("Arithmetic overflow occurred in phase_mint_aether.")?;
        mint_AETHER_internal(phase_mint_aether, actual_caller).await?; //mint to caller
    ...
    TOTAL_AETHER_MINTED.with(|mint| -> Result<(), String> {
        let mut mint: std::sync::MutexGuard<u64> = mint.lock().unwrap();
        *mint = mint
            .checked_add(minted_aether)
            .ok_or("Arithmetic overflow occurred in TOTAL_AETHER_MINTED")?;
        ic_cdk::println!("Total AETHER minted is {}", *mint);
        Ok(())
    })?;
    ...
    Ok("Minted AETHER ".to_string() + &minted_aether.to_string())
}

#[ic_cdk::update]
async fn mint_AETHER_internal(minted_aether: u64, destinaion: Principal) -> Result<BlockIndex, String> {
    ic_cdk::println!("minting to {}==>{}", minted_aether, destinaion.to_string());
    let transfer_args: TransferArg = TransferArg {
        amount: minted_aether.into(),
