use crate::{aether_principal, lbry_principal, get_nft_principal};
use crate::utils::{check_query_batch_size, to_nft_subaccount};
use crate::types::{TokenBalances, NftCollection};
...
const AETHER_FEE: u64 = 10_000;


#[update(guard = "is_frontend")]
pub async fn total_supply(collection: NftCollection) -> Result<Nat, String> {
...
#[update(guard = "is_frontend")]
pub async fn get_nfts(collection: NftCollection, start: Option<Nat>, end: Option<Nat>) -> Result<Vec<Nat>, String> {
    let total_supply = total_supply(collection).await?;
...
pub async fn get_nft_balances(mint_numbers: Vec<Nat>) -> Result<Vec<TokenBalances>, String> {
    ...
    for mint_number in mint_numbers {
        ...
        // Get the AETHER balance
        let aether_balance = ic_cdk::call::<(Account,), (NumTokens,)>(
            aether_principal(),
            "icrc1_balance_of",
            (account,),
        )
        .await
        .map_err(|e| format!("Failed to get AETHER balance for NFT {}: {:?}", mint_number, e))?
        .0;

        let lbry_fee = NumTokens::from(LBRY_FEE);
        let aether_fee = NumTokens::from(AETHER_FEE);

        results.push(TokenBalances {
            lbry: if lbry_balance > lbry_fee { lbry_balance - lbry_fee } else { NumTokens::from(0u64) },
            aether: if aether_balance > aether_fee { aether_balance - aether_fee } else { NumTokens::from(0u64) },
        });
    }
...
#[update(guard = "is_frontend")]
pub async fn nfts_exist(collection: NftCollection, token_ids: Vec<Nat>) -> Result<Vec<bool>, String> {
...
#[update(guard = "is_frontend")]
pub async fn owner_of(collection: NftCollection, token_ids: Vec<Nat>) -> Result<Vec<Option<Account>>, String> {
    check_query_batch_size(&token_ids)?;
...
#[update(guard = "is_frontend")]
pub async fn get_nfts_of(collection: NftCollection, owner: Principal) -> Result<Vec<(Nat, Option<String>)>, String> {
    let account = Account {
        owner,
        subaccount: None,
    };
    ...
    match tokens_call_result {
        Ok((token_ids,)) => {
            let manifests = get_manifest_ids(collection, token_ids.clone()).await?;
...
#[update(guard = "is_frontend")]
pub async fn get_metadata(collection: NftCollection, token_ids: Vec<Nat>) -> Result<Vec<Option<BTreeMap<String, Value>>>, String> {
    check_query_batch_size(&token_ids)?;
...
#[update(guard = "is_frontend")]
pub async fn get_manifest_ids(collection: NftCollection, token_ids: Vec<Nat>) -> Result<Vec<Option<String>>, String> {
    let manifests = get_metadata(collection, token_ids).await?.into_iter().map(|metadata| {
        metadata.and_then(|m| {
...
#[update(guard = "is_frontend")]
pub async fn can_attack(collection: NftCollection, token_ids: Vec<Nat>) -> Result<Vec<bool>, String> {
    check_query_batch_size(&token_ids)?;
    ic_cdk::println!("Checking verification status for token_ids: {:?}", token_ids);

    let exists_results = nfts_exist(collection, token_ids.clone()).await?;
    ...
    match metadata_call_result {
        Ok((metadata,)) => {
            ...
            let attackable_statuses: Vec<bool> = metadata.into_iter()
                .map(|token_metadata| {
                    if let Some(token_metadata) = token_metadata {
                        if let Some(Value::Blob(blob)) = token_metadata.get("icrc7:metadata:attackable") {
                ...
                .collect();
            ic_cdk::println!("Parsed attackable statuses: {:?}", attackable_statuses);
            Ok(attackable_statuses)
        },
        Err((code, msg)) => {
...
#[update(guard = "is_frontend")]
pub async fn get_my_nft_balances(collection: NftCollection, slot: Option<Nat>) -> Result<Vec<(Nat, TokenBalances)>, String> {
    let caller = ic_cdk::api::caller();
    
    let nfts = get_nfts_of(collection, caller).await
